$(document).ready(function(){
    /*
    header에 fixed 클래스주는것...
    1.브라우저가 조금이라도 스크롤이 되면 header에 fixed클래스 추가
        스크롤 값이 0보다 크면 (0포함 안함)
    2.다시 브라우저 맨 위로 스크롤이 되면 header에 fixed클래스 삭제
        스크롤 값이 0 이거나 그보다 작으면

    클래스를 주는 명령어

    브라우저가 스크롤되는 것을 체크할수 있는 명령을 알아야함
